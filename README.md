...
- **4 级 AI 难度**：菜鸟 / 老手 / 大师 / 恶雄（Debug 解锁）
...
FEN 序列化采用三进制棋盘 + 十六进制元数据的紧凑编码，单次序列化开销极低。
...
- **哈希增量更新**：`makeMove`/`undoMove` 时通过 `pow3[]` 增量维护位置哈希，无需全量重算
...
大师模式按阶段调整：放置深度 3 / 走子深度 4 / 飞行深度 5。恶雄模式在 Debug 解锁后可用，完全确定性（温度 0，TopK 1）。
...
MILL-FEN 是一个 JSON 字符串，包含两个字段：

```json
{"board":886243,"meta":"0x0c815"}
```
...
# 恶雄深度测试
node test/battle.js Demon Master 1
